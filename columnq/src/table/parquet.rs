...
pub async fn to_mem_table(t: &TableSource) -> Result<Arc<dyn TableProvider>, ColumnQError> {
    let lazy = match &t.option {
        Some(TableLoadOption::parquet { lazy }) => lazy.unwrap_or(false),
        _ => false,
    };

    let table: Arc<dyn TableProvider> = if lazy {
        Arc::new(ParquetTable::try_new(t.parsed_uri()?, 4).map_err(|err| {
            ...
            partitions_from_table_source!(t, |mut r| -> Result<Vec<RecordBatch>, ColumnQError> {
                ...
                schema = Some(match &schema {
                    Some(s) if s != &batch_schema => {
                        Schema::try_merge(vec![s.clone(), batch_schema])?
                    }
                    _ => batch_schema,
                });
