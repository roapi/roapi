...
async fn gs_api_get(token: &str, url: &str) -> Result<reqwest::Response, LoadTableError> {
    Client::builder()
        .build()
        .map_err(|e| {
            LoadTableError::GoogleSpeadsheets(format!(
...
// type DataTypeDate = DataType::Date32(DateUnit::Day);
// type DataTypeDateTime = DataType::Date64(DateUnit::Millisecond);

fn coerce_type(l: DataType, r: DataType) -> DataType {
    match (l, r) {
        (DataType::Boolean, DataType::Boolean) => DataType::Boolean,
        (DataType::Date32(DateUnit::Day), DataType::Date32(DateUnit::Day)) => {
            DataType::Date32(DateUnit::Day)
        }

        (DataType::Date64(DateUnit::Millisecond), DataType::Date64(DateUnit::Millisecond))
        | (DataType::Date64(DateUnit::Millisecond), DataType::Date32(_))
        | (DataType::Date32(_), DataType::Date64(DateUnit::Millisecond)) => {
...
fn infer_schema(rows: &[Vec<String>]) -> Schema {
    ...
    let fields: Vec<Field> = col_names
        ...
        .map(|col_name| {
            ...
            let dt = match set.iter().cloned().fold_first(coerce_type) {
                Some(t) => t,
                None => DataType::Utf8,
            };
...
fn sheet_values_to_record_batch(values: &[Vec<String>]) -> Result<RecordBatch, LoadTableError> {
    ...
    let arrays = schema
            ...
            Ok(match field.data_type() {
                ...
                DataType::Int64 => Arc::new(
                    rows_iter
                        ...
                        .collect::<Result<PrimitiveArray<Int64Type>, LoadTableError>>()?,
                ) as ArrayRef,
                DataType::Float64 => Arc::new(
                    rows_iter
                        ...
                        .collect::<Result<PrimitiveArray<Float64Type>, LoadTableError>>()?,
                ) as ArrayRef,
        ...
        .collect::<Result<Vec<ArrayRef>, LoadTableError>>()?;
...
) -> Result<yup_oauth2::AccessToken, LoadTableError> {
...
async fn resolve_sheet_title<'a, 'b, 'c, 'd>(
    ...
    let sheet = match sheet_id {
        Some(id) => spreadsheets
            ...
            .ok_or_else(|| LoadTableError::GoogleSpeadsheets(format!("Invalid sheet id {}", id)))?,
        ...
        None => spreadsheets
            ...
            .ok_or_else(|| {
                LoadTableError::GoogleSpeadsheets("spreadsheets is empty".to_string())
            })?,
    };
...
pub async fn to_mem_table(
    t: &TableSource,
) -> Result<datafusion::datasource::MemTable, LoadTableError> {
    lazy_static! {
    ...
    if RE_GOOGLE_SHEET.captures(&t.uri).is_none() {
        return Err(LoadTableError::InvalidUri(t.uri.to_string()));
    }
    ...
    let opt = t
        .option
        .as_ref()
        .ok_or(LoadTableError::MissingOption)?
...
mod tests {
    ...
    #[test]
    fn sheetvalue_to_memtable() -> anyhow::Result<()> {
        let sheet = property_sheet();
        let batch = sheet_values_to_record_batch(&sheet.values)?;
